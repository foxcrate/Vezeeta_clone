...
<div class="d-flex bg-page" id="wrapper">
    ...
    <div class="container mb-5">
        ...
        @endif
        <div class="row pt-5">
            <div class="col-md-9 slide-img mb-5 mr-auto ml-auto ">
            ...
            <!-- Button trigger modal -->
                <div class="text-center">
                    ...
                    <i class="fas fa-eye mr-2"></i> Show Patient Lab
                    </button>
                </div>
                <!-- Modal -->
                    ...
                    <div class="modal-dialog modal-lg">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="pills-main pills-main-yellow col-xl-8 col-md-4 col-xs-12 row mb-4 mr-auto ml-auto">
                                ...
                                <div class="col-10">
                                    <form action="{{route('add_result_ray')}}" method="POST" enctype="multipart/form-data">
                                        {{csrf_field()}}
                                        <input type="hidden" name="patient_id" value="{{$patient->id}}">
                                        @if($patient->patient_rays)
                                            @foreach($patient->patient_rays as $patient_rays)
                                                @foreach($patient_rays->ray_name as $key => $r)
                                                    <input @if($patient_rays->link) disabled @endif name = "ray_id[]" value="{{ $patient_rays->id }}" type="checkbox" class="mr-2"><span class="font-weight-bold">{{$r['ray_name']}}</span><br>
                                                    <input type="hidden" name="name[$key]" value="{{ $r['ray_name'] }}">
                                                @endforeach
                                            @endforeach
                                            @else
                                            <div class="alert alert-danger">No Ridelogy</div>
                                        @endif
                                        <input type="file" class="form-control mt-3 mb-3" name = "urlLink">
                      ...
                      <div class="col-md-12">
                        <div class="pills-main pills-main-yellow col-xl-8 col-md-4 col-xs-12 row mb-4 mr-auto ml-auto">
                          ...
                          <div class="row col-12 mb-3">
                            ...
                            <div class="col-10">
                                <form action="{{route('lab_child_add_Result_rays',$labs->id)}}" method="POST" enctype="multipart/form-data">
                                    {{csrf_field()}}
                                    @if($patient->childern)
                                        @foreach($patient->childern as $ch)
                                            <input type="hidden" name="child_id" value="{{$ch->id}}">
                                            @foreach($ch->ray_child as $r)
                                            <h5 class="text-capitalize">{{ $ch->child_name }}</h5>
                                                @foreach($r['ray_name'] as $key => $ray_name)
                                                    <input @if($r->link) disabled @endif name = "ray_child_id[]" value="{{$r->id}}" type="checkbox" class="mr-2"><span class="font-weight-bold">{{$ray_name['ray_name']}}</span><br>
                                                @endforeach
                                            @endforeach
                                        @endforeach
                                    @else
