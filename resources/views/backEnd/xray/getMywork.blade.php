...
<div class="d-flex bg-veiwdoctor" id="wrapper">
    <div id="page-content-wrapper">
        <nav class="navbarp navbar-top navbar-expand navbar-dark">
            <div class="container-fluid">
              <div class="collapse navbar-collapse" id="navbarSupportedContent">
                ...
                {{--  <h6 class="h5 text-white">{{$xray->online == 1 ? 'online' : 'ofline'}}</h6>  --}}
                    ...
                    <a class="nav-link pr-0" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                      <div class="media align-items-center">
                        <span class="avatar avatar-sm rounded-circle">
                            ...
                            @else
                                <img alt="Image placeholder" src="{{ $xray->image }}" width="50" height="40">
                            @endif
                        </span>
        ...
        <div class="container mt-4">
            ...
            @php
               $count =  \App\models\AppointmentLab::whereNull('lab_id')->count();
            @endphp
            @if($count === 0)
            <div class="form-mywork mt-3">
                <form class="row col-10 ml-auto mr-auto" action="{{route('xray.postMywork')}}" method="POST">
                    {{ csrf_field() }}
                    <input type = "hidden" name="xray_id" value="{{$xray->id}}">
                    <input type = "hidden" name="idCode" value="{{$xray->idCode}}">
                    <input type="hidden" name="xrayName" value="{{$xray->xrayName}}">
            ...
            @endif
                 @foreach($xray->appointments_lab as $appoin)
                    <div class="label-work p-3 mb-5 mt-5">
                        <div class="row col-10 ml-auto mr-auto text-center border-bottom pb-3">
                            <div class="col-lg-1 text-capitalize mt-3 font-weight-bold h5">{{$appoin->doctor_name}}</div>
                            <div class="col-lg-8 font-weight-bold mt-3 h6">{{$appoin->address}}</div>
                        ...
                        <div class="row col-10 ml-auto mr-auto mt-3 text-center">
                            @foreach($appoin->appointments as $apppp)
                                <div class="text-primary h5 font-weight-bold col-lg-4">{{$apppp['day_name']}}</div>
                                <div class="col-lg-3 h5 font-weight-bold">{{$apppp['from']}}</div>
                                <div class="col-lg-3 h5 font-weight-bold">{{$apppp['to']}}</div>
                            @endforeach
