        ...
        <div class="col-lg-6 mb-5 mr-auto ml-auto">
          ...
          <div class="text-center">
            <button type="button" class="btn btn-primary text-white col-8 h5" data-toggle="modal" data-target="#Testing">
              <i class="fas fa-eye mr-2"></i> Show Patient Test
            </button>
          </div>
          <!-- Modal -->
                      ...
                      <div class="pills-main-yellow col-xl-10 col-md-4 col-xs-12 row mb-4 mr-auto ml-auto">
                        ...
                        <div class="row col-12 mb-3">
                          ...
                          <div class="col-10 ml-auto mr-auto">
                              <form action="{{route('add_result_test',$xray->id)}}" method="POST" enctype="multipart/form-data">
                                  {{csrf_field()}}
                                    <input type="hidden" name="patient_id" value="{{$patient->id}}">
                                    @if($patient->patient_analzes)
                                        @foreach($patient->patient_analzes as $test)
                                            @foreach($test->test_name as $key => $t)
                                                <input @if($test->link) disabled @endif name = "test_id[]" value="{{$test->id}}" type="checkbox" class="mr-2"><span class="font-weight-bold">{{$t['test_name']}}</span>
                                                <input type="hidden" name="name" value="{{ $t['test_name'] }}"><br>
                                            @endforeach
                                        @endforeach
                                    @else
                                        <div class="alert alert-danger ">No Test</div>
                      ...
                      <div class="pills-main-yellow col-xl-10 col-md-4 col-xs-12 row mb-4 mr-auto ml-auto">
                        ...
                        <div class="row col-12 mb-3">
                          ...
                          <div class="col-10 ml-auto mr-auto">
                              ...
                              <form action="{{route('xray_child_add_Result_test',$xray->id)}}" multiple="multiple" method="POST" enctype="multipart/form-data">
                                  {{csrf_field()}}
                                  @if($patient->childern)
                                      ...
                                      <input type="hidden" name="child_id" value="{{$ch->id}}">
                                        @foreach($ch->test_child as $t)
                                          ...
                                          @foreach($t['test_name'] as $key => $test_name)
                                            <input @if($t->link) disabled @endif name = "test_child_id[]" value="{{$t->id}}" type="checkbox" class="mr-2"><span class="font-weight-bold">{{$test_name['test_name']}}</span><br>
                                            {{-- <input type="hidden" name="name[]" value="{{ $t['test_name'] }}"><br> --}}
                                            @endforeach
